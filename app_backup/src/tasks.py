...
@app.task
def sync_databases(worker_db):
    worker_engine = create_engine(worker_db['uri'])
    worker_session = sessionmaker(bind=worker_engine)()
    worker_news = worker_session.query(News).all()
    for news in worker_news:
        try:
            main_news = News(url=news.url, title=news.title, content=news.content)
            session.add(main_news)
            session.commit()
        except exc.IntegrityError:
            session.rollback()